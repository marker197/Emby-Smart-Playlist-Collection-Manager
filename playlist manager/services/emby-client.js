...
// Record a collection in the smart registry (direct file write, same process as server).
// Safe to call repeatedly: updates the entry if the ID already exists.
...
class EmbyClient {
  ...
  async createOrUpdateCollectionByName(name, itemIds, description = '') {
    try {
      ...
      if (existing && existing.Id) {
        ...
        const oldItemObjs = await this.getCollectionItems(existing.Id);
        const oldIds = oldItemObjs.map(i => i.Id).filter(Boolean);
  ...
  async getCollectionItems(collectionId) {
    try {
      // Use ParentId endpoint with proper axios params format
      const params = {
